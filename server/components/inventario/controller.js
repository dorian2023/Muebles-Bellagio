...
function addProduct(product) {
    return new Promise((resolve, reject) => {
        if (Object.entries(product).length === 0) {
            console.log("[InventarioController]: El producto no tiene contenido, el producto está vacío")
...
function getOnlyProductByID(id) {
    return new Promise((resolve, reject) => {
        if (!id) {
            console.log("[Controlador de Inventario]: El producto no tiene título para la búsqueda, el título del producto está vacío")
            reject('There is no title product')
        }
